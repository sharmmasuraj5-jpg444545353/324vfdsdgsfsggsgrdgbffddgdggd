...
async def stream(
    ...
    if streamtype == "playlist":
        ...
        for search in result:
            ...
            if await is_active_chat(chat_id):
                ...
                run = await app.send_photo(original_chat_id, photo=img, caption=_["stream_1"].format(f"https://t.me/{app.username}?start=info_{vidid}", title[:23], duration_min, user_name), reply_markup=InlineKeyboardMarkup(button), disable_web_page_preview=True)
        ...
        if count == 0:
            ...
            upl = close_markup(_)
            return await app.send_photo(original_chat_id, photo=carbon, caption=_["play_21"].format(position, link), reply_markup=upl, disable_web_page_preview=True)
    ...
    # ------------------------- YOUTUBE -------------------------
        ...
        if await is_active_chat(chat_id):
            ...
            await app.send_message(chat_id=original_chat_id, text=_["queue_4"].format(position, title[:27], duration_min, user_name), reply_markup=InlineKeyboardMarkup(button), disable_web_page_preview=True)
        else:
            if not forceplay:
                db[chat_id] = []
            await Sona.join_call(chat_id, original_chat_id, file_path, video=status, image=thumbnail)
    ...
    # ------------------------- SOUNDCLOUD -------------------------
        ...
        if await is_active_chat(chat_id):
            ...
            button = stream_markup(_, chat_id)
            run = await app.send_photo(original_chat_id, photo=config.SOUNCLOUD_IMG_URL, caption=_["stream_1"].format(config.SUPPORT_CHAT, title[:23], duration_min, user_name), reply_markup=InlineKeyboardMarkup(button), disable_web_page_preview=True)
            db[chat_id][0]["mystic"] = run
    ...
    # ------------------------- TELEGRAM -------------------------
        ...
        if await is_active_chat(chat_id):
            ...
            button = stream_markup(_, chat_id)
            run = await app.send_photo(original_chat_id, photo=config.TELEGRAM_VIDEO_URL if video else config.TELEGRAM_AUDIO_URL, caption=_["stream_1"].format(link, title[:23], duration_min, user_name), reply_markup=InlineKeyboardMarkup(button), disable_web_page_preview=True)
            db[chat_id][0]["mystic"] = run
            db[chat_id][0]["markup"] = "tg"
    ...
    # ------------------------- INDEX -------------------------
        ...
        if await is_active_chat(chat_id):
            ...
            position = len(db.get(chat_id)) - 1
            button = aq_markup(_, chat_id)
            await mystic.edit_text(text=_["queue_4"].format(position, title[:27], duration_min, user_name), reply_markup=InlineKeyboardMarkup(button), disable_web_page_preview=True)
        else:
            if not forceplay:
                db[chat_id] = []
            ...
            button = stream_markup(_, chat_id)
            run = await app.send_photo(original_chat_id, photo=config.STREAM_IMG_URL, caption=_["stream_2"].format(user_name), reply_markup=InlineKeyboardMarkup(button), disable_web_page_preview=True)
            db[chat_id][0]["mystic"] = run
            db[chat_id][0]["markup"] = "tg"
            await mystic.delete()
